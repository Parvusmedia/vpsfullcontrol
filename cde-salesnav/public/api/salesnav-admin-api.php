...
if ($action === 'status' && $_SERVER['REQUEST_METHOD'] === 'GET') {
    cde_json_response(200, [
        'ok' => true,
        'authenticated' => cde_admin_session_valid() || cde_admin_token_valid(trim((string) ($_SERVER['HTTP_X_SALESNAV_ADMIN_TOKEN'] ?? ''))),
        'admin_configured' => cde_admin_configured(),
    ]);
}

if ($action === 'login' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!cde_admin_configured()) {
...
if ($action === 'overview' && $_SERVER['REQUEST_METHOD'] === 'GET') {
    cde_json_response(200, ['ok' => true, 'overview' => cde_admin_overview()]);
}
...
if ($action === 'user' && $_SERVER['REQUEST_METHOD'] === 'GET') {
    $userId = cde_admin_resolve_user_id($_GET['user_id'] ?? '', $_GET['email'] ?? '');
    if ($userId === null) {
        cde_json_response(400, ['ok' => false, 'error' => 'user_id or email required']);
    }
    $detail = cde_admin_user_detail($userId);
    if ($detail === null) {
...
if ($action === 'ledger' && $_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    cde_json_response(200, [
        'ok' => true,
        'ledger' => cde_admin_read_ledger($limit, $userId, $kind === 'all' ? null : $kind),
    ]);
}
...
if ($action === 'tasks' && $_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    cde_json_response(200, [
        'ok' => true,
        'tasks' => cde_admin_list_tasks($limit, $userId, $status === 'all' ? null : $status),
    ]);
}
...
if ($action === 'grant' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $result = cde_admin_grant_credits($email, $amount, $note, $ref);
