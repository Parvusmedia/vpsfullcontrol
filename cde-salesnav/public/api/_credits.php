...
function cde_credits_promo_redemptions_file(): string
{
    $dir = cde_salesnav_private_dir();
    return $dir . '/salesnav_promo_redemptions.json';
}

function cde_credits_load_promo_redemptions(): array
{
    $path = cde_credits_promo_redemptions_file();
    if (!is_readable($path)) {
        return [];
    }
    $data = json_decode((string) file_get_contents($path), true);
...
function cde_credits_save_promo_redemptions(array $rows): void
{
    $path = cde_credits_promo_redemptions_file();
...
function cde_credits_has_redeemed_promo(string $userId, string $promoId): bool
{
    $rows = cde_credits_load_promo_redemptions();
    foreach ($rows as $row) {
        if (($row['user_id'] ?? '') === $userId && ($row['promo_id'] ?? '') === $promoId) {
            return true;
        }
    }
    return false;
...
function cde_credits_mark_promo_redeemed(string $userId, string $promoId, string $code): void
{
    $rows = cde_credits_load_promo_redemptions();
    $rows[] = [
        'user_id' => $userId,
    ...
    cde_credits_save_promo_redemptions($rows);
}

function cde_credits_user_has_paid_before(string $userId): bool
{
    ...
    while (($line = fgets($handle)) !== false) {
        ...
        if (!is_array($entry) || ($entry['user_id'] ?? '') !== $userId) {
            continue;
        }
        $ref = (string) ($entry['ref'] ?? '');
        if (strpos($ref, 'stripe:') === 0) {
            fclose($handle);
            return true;
        }
    }
