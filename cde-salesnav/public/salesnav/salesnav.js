...
const I18N = {
  en: {
    ...
    "credits.exportTopupDetail":
      "This export needs {required} credits (you have {balance}). You need {shortfall} more. Top up with {packCredits} credits ({packPrice}) to continue.",
  ...
  es: {
    ...
    "credits.exportTopupDetail":
      "Este export necesita {required} créditos (tienes {balance}). Te faltan {shortfall}. Recarga {packCredits} créditos ({packPrice}) para continuar.",
...
async function startExportTopupCheckout(packId) {
  ...
  try {
    const paid = await startStripeCheckout(packId);
    if (!paid) {
      return;
    }
    await retryPendingExportIfReady();
