...
echo "=== using $BASE ==="
echo "=== headers ==="
curl -sS "${INSECURE[@]}" -D - -o /dev/null "${H[@]}" "$BASE/fares/MAD.json" | tr -d '\r' | tee /tmp/live-fare-headers.txt
python3 - <<'PY'
...
curl_h "$BASE/creative/index.html" | grep -q "Riyadh Air"
curl_h "$BASE/creative/app.js" | grep -q "feedUrl"
...
req = urllib.request.Request(base + "/fares/MAD.json", headers={"Host": "flights.pmediaplus.com"})
...
code="$(curl -sS "${INSECURE[@]}" -o /dev/null -w "%{http_code}" "${H[@]}" "$BASE/fares/MAD.json" || true)"
