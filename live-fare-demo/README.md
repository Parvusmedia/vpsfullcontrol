...
| Feed | https://flights.pmediaplus.com/fares/MAD.json |
| Creative | https://flights.pmediaplus.com/creative/ |
| Health | https://flights.pmediaplus.com/health |

DNS necesario (Plesk / `ns1.pmediaplus.com`):

```text
flights.pmediaplus.com  A  87.106.194.137
```

Sin ese registro, Certbot no puede emitir HTTPS. El vhost HTTP queda instalado y se puede re-ejecutar el deploy cuando el DNS exista.
...
source/fares.json
    ...
    → write MAD.tmp.json + os.replace → MAD.json
    → Nginx (CORS *, Cache-Control max-age=30)
    → HTML5 fetch() una vez
    → dropdowns resuelven en memoria
```
...
Fichero público: `/opt/apps/live-fare-demo/feed/public/MAD.json`

Fuente (precios base y rangos): `feed/source/fares.json`

Origen MAD. Destinos RUH / JED / DXB. Meses 2026-10, 2026-11, 2026-12.
...
./scripts/set-fare.sh MAD RUH 2026-10 299
```
...
Vanilla HTML/CSS/JS. Un `fetch` al cargar. Cambios de destino/mes no disparan red.
...
El ZIP contiene solo `index.html`, `styles.css`, `app.js` (y `assets/` si hay ficheros). Peso actual: **4.7 KB**. El creative llama al feed remoto por HTTPS. Sin Studio, sin API keys.
