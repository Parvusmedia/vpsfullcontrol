...
def _fare_badge(lang: Lang, quote: FareQuote) -> str:
    if quote.is_exact and quote.source in ("amadeus", "saudia_scrape", "mock"):
...
def build_results_caption(
    ...
    *,
    ...
    if exact:
        ...
        if not exact[0].is_exact or exact[0].source == "network_month_floor":
            parts.append("")
        ...
        if not flex[0].is_exact or flex[0].source == "network_month_floor":
            parts.append("")
            parts.append(t("result_indicative", lang))
        ...
        rows.append(
            [
                InlineKeyboardButton(
                    t("book_button", lang, sar=f"{q.price_sar:,}"),
                    url=q.book_url,
                )
            ]
        )
