...
def _education_score(profile: dict[str, Any], programme: dict[str, Any]) -> tuple[float, list[str]]:
    ...
    if score >= 0.7 and profile.get("education"):
        reasons.append("Your degree aligns with catalogue access profiles")
    elif score >= 0.4 and profile.get("education"):
        reasons.append("Partial overlap with access profiles")
...
def _modality_score(profile: dict[str, Any], programme: dict[str, Any]) -> tuple[float, list[str]]:
    ...
    if "combine work and study" in constraints:
        if programme.get("work_compatible") or str(programme.get("modality", "")).lower() == "hybrid":
            reasons.append("Hybrid delivery may fit if you keep working")
            return 1.0, reasons
        reasons.append("On-campus delivery may be harder to combine with work")
        return 0.25, reasons
    return 0.6, reasons
...
def score_programme(
    ...
    return {
        ...
        "modality_label": "Hybrid" if str(programme.get("modality")).lower() == "hybrid" else "On campus",
        "ects": programme["ects"],
