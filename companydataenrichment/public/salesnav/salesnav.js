...
const I18N = {
  en: {
    ...
    "account.guestLead": "Sign in or create an account to manage credits and exports.",
  ...
  es: {
    ...
    "account.guestLead": "Inicia sesión o crea una cuenta para gestionar créditos y exports.",
...
async function signInFromForm(ev) {
  if (ev?.preventDefault) ev.preventDefault();
  const emailInput = document.getElementById("sign-in-email") || document.getElementById("billing-email");
...
async function registerFromForm(ev) {
  if (ev?.preventDefault) ev.preventDefault();
  ...
  try {
    ...
    setAccountNote(data.needs_verification ? t("account.registerOk") : t("account.registerReady"), "ok");
  } catch (err) {
    setAccountNote(err.message || t("msg.generic"), "error");
  }
}
...
async function resendVerificationFromForm() {
  const email = (document.getElementById("sign-in-email")?.value || document.getElementById("register-email")?.value || "").trim();
  if (!email) {
    setAccountNote(t("credits.emailRequired"), "error");
    return;
  }
...
function initAuthTabs() {
  document.querySelectorAll("[data-auth-tab]").forEach((btn) => {
    btn.addEventListener("click", () => setAuthTab(btn.getAttribute("data-auth-tab") || "signin"));
  });
}
...
function initPanelPage() {
  ...
  document.getElementById("sign-in-form")?.addEventListener("submit", (e) => signInFromForm(e));
  document.getElementById("register-form")?.addEventListener("submit", (e) => registerFromForm(e));
  document.getElementById("resend-verify-btn")?.addEventListener("click", () => resendVerificationFromForm());
