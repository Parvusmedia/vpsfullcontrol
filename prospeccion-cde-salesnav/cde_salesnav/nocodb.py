...
def patch_record(*, cfg: CdeConfig, record_id: int, fields: dict[str, Any]) -> None:
    resp = httpx.patch(
        f"{cfg.nocodb_base_url}/api/v2/tables/{cfg.nocodb_table_id}/records",
        headers=_headers(cfg),
        json={"Id": record_id, **fields, "last_touch_at": _utcnow()},
        timeout=45,
    )
    resp.raise_for_status()
...
def compose_messages(
    *,
    cfg: CdeConfig | None = None,
    relevante: str = "Pendiente",
    force: bool = False,
    dry_run: bool = True,
    row_ids: list[int] | None = None,
) -> dict[str, Any]:
    """Generate LinkedIn connection + follow-up drafts for review in NocoDB."""
    from .messages import compose_row_messages
    ...
    if row_ids:
        ...
        rows = [r for r in rows if str(r.get("relevante") or "").strip() == relevante]
    ...
    for row in rows:
        ...
        if rid is None:
            continue
        if not force and str(row.get("connection_message") or "").strip():
            skipped.append({"id": rid, "title": row.get("Title"), "reason": "has_message"})
            continue
        payload = compose_row_messages(row)
        item = {
            "id": rid,
            "title": row.get("Title"),
            "mensaje_estado": payload["mensaje_estado"],
            "connection_preview": payload["connection_message"][:120] + "…",
        }
        if dry_run:
            updated.append(item)
            continue
        try:
            patch_record(cfg=cfg, record_id=int(rid), fields=payload)
            updated.append(item)
        except Exception as exc:
            skipped.append({"id": rid, "title": row.get("Title"), "reason": str(exc)[:200]})
