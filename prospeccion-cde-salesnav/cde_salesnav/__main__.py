"""CLI: queries | discover | ensure-schema | sync | compose-messages."""
...
def cmd_compose_messages(
    ...
    *,
    ...
    from .nocodb import compose_messages

    out = compose_messages(
        cfg=cfg,
        relevante=relevante,
        force=force,
        dry_run=not live,
...
def main(argv: list[str] | None = None) -> int:
    ...
    cm = sub.add_parser("compose-messages", help="Draft LinkedIn connection + follow-up in NocoDB")
    cm.add_argument("--live", action="store_true", help="Write messages to NocoDB")
    cm.add_argument("--force", action="store_true", help="Overwrite existing connection_message")
    cm.add_argument("--relevante", default="Pendiente", help="Filter by relevante (default Pendiente)")
    cm.add_argument("--ids", type=int, nargs="*", default=[], help="Only these NocoDB row Ids")
    ...
    if args.cmd == "compose-messages":
        return cmd_compose_messages(
            cfg,
            live=args.live,
            force=args.force,
            relevante=args.relevante,
            ids=args.ids,
        )
    return 2
